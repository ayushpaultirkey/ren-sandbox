...
class Project extends H12 {
    ...
    async openWorkspace(path, data) {
        ...
        tabManager.addTab(uuid, path);

        this.set("{workspaces}++", <>
            <workspace args alias={ module } id={ uuid } data={ data }></workspace>
        </>);
        
        this.switchWorkspace(uuid);
    ...
    load() {
        try {
            ...
            this.openWorkspace("bruh.ren", data);
